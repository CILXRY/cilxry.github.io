...
### 基础信息
- `category` - **改为单数**（原为 `categories`）
- `author?: string` - 作者
- `draft?: boolean` - 是否草稿（默认 false）

### 简介信息
- `descGenAuthor?: string` - description 生成者（如 "AI", "Manual"）
- `descGenTime?: string | Date` - description 生成时间

### 内容信息
- `warning?: string` - 内容警告
- `original?: boolean` - 原创标识（默认 true）
...
```typescript
import { formatDate, formatDateTime, normalizeDate } from "./frontmatter/utils/formatDate.js";
